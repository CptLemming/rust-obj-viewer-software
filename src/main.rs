...
fn main() {
    let mut window = Window::new("3D", 512, 512);
    let mut depth_buffer = FrameBuffer::new(window.buffer().width(), window.buffer().height());

    // let model = Model::load("assets/BoomBox/BoomBox.gltf");
    let model = Model::load("assets/DamagedHelmet/DamagedHelmet.gltf");

    // let timer = SystemTime::now();

    while !window.should_close() {
        let zoom = window.zoom() + 60.0;
        let rotate = window.rotate();

        let buffer = window.buffer();

        if buffer.width() != depth_buffer.width() || buffer.height() != depth_buffer.height() {
            depth_buffer = FrameBuffer::new(buffer.width(), buffer.height());
        }

        buffer.clear(from_u8_rgb(20, 20, 20));
        depth_buffer.clear(u32::MAX);

        let aspect_ratio = buffer.width() as f32 / buffer.height() as f32;
        let model_matrix = Mat4::from_axis_angle(Vec3::new(0.0, 1.0, 0.0), rotate.x)
            * Mat4::from_axis_angle(Vec3::new(1.0, 0.0, 0.0), (rotate.y + 90.0).to_radians());
        let view_matrix = Mat4::from_translation(Vec3::new(0.0, 0.0, -2.5));
        let projection_matrix = Mat4::perspective_rh(zoom.to_radians(), aspect_ratio, 0.01, 300.0);
...
fn project(p: &Vec3, mvp: &Mat4) -> (Vec3, f32) {
    let project_position = *mvp * Vec4::from((*p, 1.0));
    let rec = 1.0 / project_position.w;
    let rec_pos = project_position * rec;
        ...
        for y in (min.y as usize)..(max.y as usize) {
            ...
            if overlaps {
                ...
                if z < depth {
                    ...
                    let n0 = *inverse_transpose_model_matrix * Vec4::from((v0.normal, 1.0));
                    let n1 = *inverse_transpose_model_matrix * Vec4::from((v1.normal, 1.0));
                    let n2 = *inverse_transpose_model_matrix * Vec4::from((v2.normal, 1.0));
                    ...
                    buffer.set_pixel(x, y, from_vec3_rgb(&final_color.xyz()));
                }
            }
        }
    }
...
fn draw_model(
    ...
    for mesh in &model.meshes {
        for i in 0..(mesh.indices.len() / 3) {
            let v0 = mesh.vertices[mesh.indices[i * 3 + 0] as usize];
